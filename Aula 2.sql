...
SELECT TREINADOR.ID_TREINADOR, TREINADOR.NOME, TREINADOR.IDADE, TREINADOR.TELEFONE, SUM(ANOS) AS "Soma Anos"
FROM EXPERIENCIA,TREINADOR
WHERE EXPERIENCIA.ID_TREINADOR=TREINADOR.ID_TREINADOR AND EXPERIENCIA.ESCALAO='JUNIORES'
GROUP BY TREINADOR.ID_TREINADOR,TREINADOR.NOME,TREINADOR.IDADE,TREINADOR.TELEFONE
HAVING SUM(ANOS) >=5
...
SELECT DISTINCT TOP(1) TREINADOR.IDADE,TREINADOR.NOME AS 'TREINADOR MAIS VELHO' FROM TREINADOR,EXPERIENCIA,EQUIPA WHERE TREINADOR.ID_TREINADOR=EXPERIENCIA.ID_TREINADOR AND EXPERIENCIA.ID_EQUIPA=EQUIPA.ID_EQUIPA AND EQUIPA.NOME='ACADÉMICO' ORDER BY IDADE DESC
